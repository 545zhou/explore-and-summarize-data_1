...
```{r echo=FALSE}
pf_temp <- pf
names(pf_temp) <- c("f.a", "v.a", "c.a", "r.s", "chl", "fsd", "tsd", "den", "pH", "sul", "alc", "qua")
ggpairs(pf_temp, 
        params = c(shape = I('.'), outlier.shape = I('.'), grid = NA), 
...
ggplot(aes(x = qualFact, y = fixed.acidity), data = pf) +
  geom_boxplot(outlier.colour = alpha("black", 0.1)) + geom_point(alpha = 0.1)
```
